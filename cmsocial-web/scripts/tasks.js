...
angular.module('cmsocial')
  .service(
    'subsDatabase',
    function($http, $rootScope, $timeout, notificationHub, l10n,
      API_PREFIX) {
  ...
  .controller(
    'TasklistPage',
    function($scope, $stateParams, $state, $http, notificationHub, l10n,
      API_PREFIX) {
      ...
      $scope.getTasks = function() {
        $http
          .post(API_PREFIX + 'task', {
            ...
            'first': $scope.pagination.perPage *
          ...
          .success(function(data, status, headers, config) {
            ...
            $scope.pagination.total =
              Math.ceil(data['num'] / $scope.pagination.perPage);
            if (data['num'] === 0) {
  ...
  .controller('TagsPage',
    function($scope, $http, notificationHub, API_PREFIX) {
  ...
  .controller(
    'HelpCtrl',
    function($scope, $stateParams, $http, notificationHub, API_PREFIX) {
      $scope.data = {
        testcase: '000',
        ...
        $http
          ...
          .success(function(data, status, headers, config) {
            if (data.success == 1) {
              ...
              input.href =
                API_PREFIX + "files/" + data['input'] + "/input.txt";
              input.onclick = function() {
                input.className = "btn btn-primary";
              };
              var output = document.getElementById("output-download");
              output.href =
                API_PREFIX + "files/" + data['output'] + "/output.txt";
