...
with con.cursor() as cur:
    ...
    cur.execute("CREATE TEMPORARY TABLE update_submission_results ("
                "   id integer, "
                "   dataset_id integer, "
                "   score_details jsonb, "
    ...
    for batch in range(100):
        ...
        psycopg2.extras.execute_values(cur, "INSERT INTO update_submission_results (id, dataset_id, score_details) VALUES %s", updated)
        duration = monotonic() - start
        print("Took %.3fs, ETA: %.3f" % (duration, duration * (100 - batch - 1)))

    print("Updating actual data")
    cur.execute("UPDATE submission_results S "
